function QuestionCategory() {
    ...
    this.initQuestionCategoriesDatatable = function () {
        $('#question_categories_datatable').DataTable({
            ...
            "ajax": {
                "type": "POST",
                "url": application.url+'/employer/question-categories/data',
                "data": function ( d ) {
                    d.status = $('#status').val();
    ...
    this.initQuestionCategorySave = function () {
        application.onSubmit('#employer_question_category_create_update_form', function (result) {
            application.showLoader('employer_question_category_create_update_form_button');
            application.post('/employer/question-categories/save', '#employer_question_category_create_update_form', function (res) {
                var result = JSON.parse(application.response);
                if (result.success === 'true') {
                    $('#modal-default').modal('hide');
                    self.initQuestionCategoriesDatatable();
                } else {
                    application.hideLoader('employer_question_category_create_update_form_button');
                    application.showMessages(result.messages, 'employer_question_category_create_update_form');
                }
            });
        });
    };
    ...
    this.initQuestionCategoryCreateOrEditForm = function () {
        ...
        $('.create-or-edit-question-category').on('click', function () {
            ...
            application.load('/employer/question-categories/create-or-edit'+id, modal+' .modal-body-container', function (result) {
    ...
    this.initQuestionCategoryChangeStatus = function () {
        ...
        $('.change-question-category-status').on('click', function () {
            ...
            application.load('/employer/question-categories/status/'+id+'/'+status, '', function (result) {
    ...
    this.initQuestionCategoryDelete = function () {
        ...
        $('.delete-question-category').on('click', function () {
            ...
            var id = $(this).data('id');
            if (status === true) {
                application.load('/employer/question-categories/delete/'+id, '', function (result) {
                    self.initQuestionCategoriesDatatable();
                });
            }
        });
    ...
    this.initQuestionCategoriesListBulkActions = function () {
        ...
        $('.bulk-action').on('click', function () {
            ...
            if (ids.length === 0) {
                ...
                application.post('/employer/question-categories/bulk-action', {ids:ids, action: $(this).data('action')}, function (result) {
                    $('.bulk-action').val('');
